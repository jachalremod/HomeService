          ...
          {estimate.payment_schedule.map((schedule: { title: string; percentage: number }, index: number) => (
            <div key={index} className="flex items-center justify-between px-4 py-3">
              <div>
                ...
                <p className="text-xs text-slate-500">{schedule.percentage}% of total</p>
              </div>
              <p className="font-bold text-slate-950">{money(Number(estimate.total) * (schedule.percentage / 100))}</p>
            </div>
          ))}
        </div>
      </section>
